...
private suspend fun executeImport(context: Context, status: (String) -> Unit): Boolean {
    return withContext(Dispatchers.IO) {
        ...
        try {
            ...
            db.execSQL("Delete from importNewTrx")
            ...
            status("Lade Partner")
            stream = am.open("partnerstamm.csv")
            FileUtils.loadCSVFile(stream, db, "partnerstamm")
            status("Lade Kategorien")
            stream = am.open("cat.csv")
            FileUtils.loadCSVFile(stream, db, "cat")
            status("Lade WPStamm")
            stream = am.open("wpstamm.csv")
            FileUtils.loadCSVFile(stream, db, "WPStamm")
            status("Lade Accounts")
            stream = am.open("account.csv")
            FileUtils.loadCSVFile(stream, db, "account")
            status("Lade ImportAccounts")
            stream = am.open("importaccount.csv")
            FileUtils.loadCSVFile(stream, db, "importaccount")
            status("Lade WPKurse")
            stream = am.open("wpkurs.csv")
            FileUtils.loadCSVFile(stream, db, "WPKurs")
            status("Lade CashTrx")
            stream = am.open("cashtrx.csv")
            FileUtils.loadCSVFile(stream, db, "CashTrx")
            status("Lade Daueraufträge")
            stream = am.open("trxregelm.csv")
            FileUtils.loadCSVFile(stream, db, "TrxRegelm")
            status("Lade WPTrx")
            stream = am.open("wptrx.csv")
            FileUtils.loadCSVFile(stream, db, "WPTrx")
            status("Nacharbeiten")
